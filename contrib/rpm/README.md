...
The RPM spec file provided here is for Meowcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://meowcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Meowcoin git repository](https://github.com/MeowcoinProject/Meowcoin) but are not part of
...
Currently only `Source10` is used, for the example `meowcoin.conf` file.
...
in the meowcoin git repository. At present only an SVG version of the Meowcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the meowcoin
...
needed when 0.12.1 is released, a proper fix is already in the Meowcoin git
master branch.
...
idea to build Meowcoin Core against a static version of that library compiled
...
The `meowcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `meowcoin-server` package. At this point, most RPM based
...
The `meowcoin-server` package should have SELinux support. How to properly do
...
Meowcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/meowcoin/). He has been
packaging Meowcoin for Fedora at least since 2012.
...
4. I renamed the `meowcoin` package that contains the Qt GUI to `meowcoin-core` as
that appears to be how the general population refers to it, in contrast to
`meowcoin-xt` or `meowcoin-classic`. I wanted to make sure the general population

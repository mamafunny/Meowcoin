...
The RPM spec file provided here is for Meow-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://meow.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Meow git repository](https://github.com/MeowProject/Meowcoin) but are not part of
...
Currently only `Source10` is used, for the example `meow.conf` file.
...
in the meow git repository. At present only an SVG version of the Meow
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the meow
...
needed when 0.12.1 is released, a proper fix is already in the Meow git
master branch.
...
idea to build Meow Core against a static version of that library compiled
...
The `meow.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `meow-server` package. At this point, most RPM based
...
The `meow-server` package should have SELinux support. How to properly do
...
Meow-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/meow/). He has been
packaging Meow for Fedora at least since 2012.
...
4. I renamed the `meow` package that contains the Qt GUI to `meow-core` as
that appears to be how the general population refers to it, in contrast to
`meow-xt` or `meow-classic`. I wanted to make sure the general population

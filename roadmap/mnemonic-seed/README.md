...
A command-line option (-noseed) can be passed when starting meowd and/or meow-qt for the first time to create a wallet using the original key system that existed in Bitcoin and Meowcoin on Jan 3, 2019.
...
When creating the wallet.dat for the first time, the default should be generating 128 bits of entropy, and storing either the words, or the 132 bits (which includes a 4 bit checkum).  If, however, the -noseed option is set via command-line or meow.conf, then the orginal method of creating a master key should be used, and the original path derivation should be used.

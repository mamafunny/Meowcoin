...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test_ipfs)
    {
        ...
        CNewAsset asset1("MEWCASSET", CAmount(100 * COIN), 8, 1, 0, "");

        // Add an asset to a valid mewc address
        ...
        CReissueAsset reissue1("MEWCASSET", CAmount(1 * COIN), 8, 1, DecodeAssetData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("MEWCASSET"), "Map Reissued Asset should contain the asset \"MEWCASSET\"");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("MEWCASSET", GetParams().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");

        // Get the new asset data from the cache
        CNewAsset asset2;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("MEWCASSET", asset2), "Failed to get the asset2");
        ...
        BOOST_CHECK_MESSAGE(asset2.strName == "MEWCASSET", "Asset2: Asset name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("MEWCASSET", CBlockAssetUndo{true, false, "", 0, ASSET_UNDO_INCLUDES_VERIFIER_STRING, false, ""}));
        ...
        CNewAsset asset3;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("MEWCASSET", asset3), "Failed to get the asset3");
        ...
        BOOST_CHECK_MESSAGE(asset3.strName == "MEWCASSET", "Asset3: Asset name is wrong");
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("MEWCASSET"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("MEWCASSET", GetParams().GlobalBurnAddress())) == CAmount(100 * COIN), "Assets total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test_txid)
    {
        ...
        // Create a reissuance of the asset
        CReissueAsset reissue1("MEWCASSET", CAmount(1 * COIN), 8, 1, DecodeAssetData("9c2c8e121a0139ba39bffd3ca97267bca9d4c0c1e84ac0c34a883c28e7a912ca"));
        COutPoint out(uint256S("BF50CB9A63BE0019171456252989A459A7D0A5F494735278290079D22AB704A4"), 1);
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CReissueAsset reissue3("MEWCASSET", CAmount(1 * COIN), 7, 1, DecodeAssetData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue4("MEWCASSET", CAmount(1 * COIN), -1, 1, DecodeAssetData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CNewAsset asset2("MEWCASSET2", CAmount(100 * COIN), 0, 1, 0, "");

        // Add new asset2 to a valid mewc address
        ...
        CReissueAsset reissue5("MEWCASSET2", CAmount(1 * COIN), 1, 1, DecodeAssetData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue6("MEWCASSET2", CAmount(1 * COIN), 1, 1, DecodeAssetData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        // Add new asset3 to a valid mewc address

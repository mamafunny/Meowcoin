...
{
    // Call into TxToUniv() in meow-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in meow-common, so we query them here and push the
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    ...
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CMeowAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CMeowAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination meow address.\n"
            ...
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/MeowProject/Meowcoin/issues/675
...
// TODO, if we decide to remove the consensus check https://github.com/MeowProject/Meowcoin/issues/675, remove or add the code (requires consensus change)
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) meow address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"

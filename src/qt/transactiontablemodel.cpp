...
//{
//    columns[Amount] = MeowUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, MeowUnits::SeparatorStyle separators) const
{
    QString str;
    switch(wtx->type) {
        case TransactionRecord::Issue:
        ...
        default:
            {
            str = MeowUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, MeowUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "MEWC")
               return QString::fromStdString(rec->assetName);
            else
               return QString(MeowUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, MeowUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, MeowUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "MEWC")
               ...
               assetName.append(QString(MeowUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:

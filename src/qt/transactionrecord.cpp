    ...
    for(size_t i=0; i < wtx.tx->vin.size(); i++)
    {
        ...
        if(fIsSingleSign)
        {
            ...
            if(fSentAssets && fRecvAssets) {
                ...
                std::string asset_qty_format = MeowcoinUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                ...
                std::string asset_qty_format = MeowcoinUnits::formatWithCustomName(QString::fromStdString(recvType), recvAmount, 2).toUtf8().constData();

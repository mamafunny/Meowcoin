...
{
    return QCoreApplication::translate("meow-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. meow_de.qm (shortcut "de" needs to be defined in meow.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. meow_de_DE.qm (shortcut "de_DE" needs to be defined in meow.qrc)
...
/** Class encapsulating Meow Core startup and shutdown.
...
static const char* qt_argv = "meow-qt";
...
{
    QMessageBox::critical(0, "Runaway exception", MeowGUI::tr("A fatal error occurred. Meow can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(meow);
    Q_INIT_RESOURCE(meow_locale);
    ...
    /// 6. Determine availability of data directory and parse meow.conf
    ...
    // meow: links repeatedly have their payment requests routed to this process:

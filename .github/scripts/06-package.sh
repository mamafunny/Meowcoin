...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/meowcoin-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="meowcoin-${VERSION}"
else
    DISTNAME="meowcoin-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert meowcoin-cli.exe meowcoin-qt.exe meowcoind.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Meowcoin-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Meowcoin-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Meowcoin-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/meowcoind .
        cp -Rf ${DISTNAME}/bin/meowcoin-cli .
    else
        echo "release directory doesn't exist"
    fi

...
The Meow Core daemon can be configured to act as a trusted "border
router", implementing the meow wire protocol and relay, making
...
The ZeroMQ feature in Meow Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building meowd:
...
For instance:
    ...
    $ meowd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/meowd.tx.raw
...
These options can also be provided in meow.conf.
...
From the perspective of meowd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into meowd directly. Furthermore, no information is
...
using. Meowd appends an up-counting sequence number to each

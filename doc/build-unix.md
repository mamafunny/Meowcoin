...
====================
Some notes on how to build Meow Core in *nix.


Note
---------------------
Always use absolute paths to configure and compile meow and the dependencies,
...
This will build meow-qt as well if the dependencies are met.
...
Notes
-----
The release is built with GCC and then "strip meowd" to strip the debug
...
Security
--------
To help make your meow installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, meow should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, meow may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/meow/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Meow Core distributions and independently built

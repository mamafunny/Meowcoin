...
# TestNode: A peer we use to send messages to meowd, and store responses.
...
class CompactBlocksTest(MeowTestFramework):
    ...
    # This test actually causes meowd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that meowd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for meowd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # meowd will not send blocktxn responses for blocks whose height is

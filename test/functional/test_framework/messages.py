...
"""
Meow test framework primitive and message structures.
...
COIN = 100000000  # 1 mewc in Corbies
...
class CAddress:
    """
    Objects that map to meowd objects, which can be serialized/deserialized
    """
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in meowd
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in meowd indicates these should be deserialized as blocks

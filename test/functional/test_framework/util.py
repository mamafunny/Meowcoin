...
x16r_hash_cmd = os.path.dirname(os.path.realpath(__file__)) + "/../../../src/test/test_meow_hash"
...
def initialize_data_dir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "meow.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "meow.conf")):
        with open(os.path.join(datadir, "meow.conf"), 'r', encoding='utf8') as f:
            for line in f:

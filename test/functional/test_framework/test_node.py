...
class TestNode:
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("MEOWCOIND", "meowcoind")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("MEOWCOINCLI", "meowcoin-cli"), self.datadir)
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "meowcoind exited with status %i during initialization" % self.process.returncode
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. meowcoind still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to meowcoind")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes meowcoind to shutdown, so this method takes
...
class TestNodeCLI:
    ...
    def __call__(self, *args, input_data=None):
        # TestNodeCLI is callable with meowcoin-cli command-line args
    ...
    def send_cli(self, command, *args, **kwargs):
        ...
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same meowcoin-cli call"

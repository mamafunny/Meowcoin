...
class MeowcoinTestFramework:
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true", help="Leave meowcoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true", help="Don't stop meowcoinds after the test execution")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"), help="Source directory containing meowcoind/meowcoin-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: meowcoind's were not stopped and may still be running")
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2 ** 16) as log_stderr:
            try:
                ...
                assert 'meowcoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "meowcoind should have exited with an error"
                else:
                    assert_msg = "meowcoind should have exited with expected error " + expected_msg
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_data_dir(self.options.tmpdir, i)  # Overwrite port/rpcport in meowcoin.conf
